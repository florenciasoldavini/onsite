  ...
  try {
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        subject: `You're invited to ${project?.name ?? "an Onzait project"}`,
        html: await renderProjectInvitationEmail({
          acceptUrl,
          expiresAt: new Date(String(invitation.expires_at)).toLocaleString(
            "en",
          ...
          inviterName:
            ...
            user.email?.split("@")[0] ||
            "A project owner",
          projectName: project?.name ?? "Onzait project",
          roleName: role?.display_name ?? roleCode
        }),
